...
-- Input paths
cmd:option('-model','model_id_MSR_VTT_challenge.t7','path to model to evaluate')
-- Basic options
cmd:option('-input_feature_dim', 27648,'the input feature size of each video.')
cmd:option('-batch_size', 1, 'if > 0 then overrule, otherwise load from checkpoint.')
cmd:option('-num_images', 497, 'how many images to use when periodically evaluating the loss? (-1 = all)')
...
cmd:option('-input_h5','data_augmentation_test.h5','path to the h5file containing the preprocessed dataset. empty = fetch from model checkpoint.')
cmd:option('-input_json','data_augmentation_test.json','path to the json file containing additional info and vocab. empty = fetch from model checkpoint.')
...
if string.len(opt.image_folder) == 0 then
  loader = DataLoader_FEAT{h5_file = opt.input_h5, json_file = opt.input_json, input_feature_dim = opt.input_feature_dim}
else
  loader = DataLoaderRaw{folder_path = opt.image_folder, coco_json = opt.coco_json, input_feature_dim = opt.input_feature_dim}
end
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local data = loader:getBatch{batch_size = opt.batch_size, split = split, seq_per_img = opt.seq_per_img, input_feature_dim = opt.input_feature_dim}
    --data.images = net_utils.prepro(data.images, false, opt.gpuid >= 0) -- preprocess in place, and don't augment
    n = n + data.images:size(1)

    -- forward the model to get loss
    local feats = protos.cnn:forward((data.images):cuda())
    ...
    local seq, logprob = protos.lm:sample(feats:cuda(), sample_opts)
    local sents = net_utils.decode_sequence(vocab, seq)
    logsum = torch.sum(logprob,1)
    for k=1,#sents do
      ...
      if verbose then
        print(string.format('image %s: %s|%f', entry.image_id,entry.caption,logsum[1][k]/#sents[k]))
      end
    end
