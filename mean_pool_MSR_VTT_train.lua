...
-- Data input settings
cmd:option('-input_h5','data_augmentation.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','data_augmentation.json','path to the json file containing additional info and vocab')
...
cmd:option('-input_feature_dim', 27648,'the input feature size of each video.')

-- Optimization: General 
-- update_iter
cmd:option('-update_iter', 4, 'number of iterations to update gradients')
cmd:option('-max_iters', -1, 'max number of iterations to run for (-1 = run forever)')
cmd:option('-batch_size', 16,'what is the batch size in number of images per batch? (there will be x seq_per_img sentences)')
cmd:option('-grad_clip', 0.1,'clip gradients at this value (note should be lower than usual 5 because we normalize grads by both batch and seq_length)')
cmd:option('-drop_prob_lm', 0.9, 'strength of dropout in the Language Model RNN')
...
cmd:option('-learning_rate', 4e-4,'learning rate') --4e-4
...
cmd:option('-cnn_optim_alpha',0.8,'alpha for momentum of CNN')--0.8
cmd:option('-cnn_optim_beta',0.999,'alpha for momentum of CNN')
cmd:option('-cnn_learning_rate',1e-5,'learning rate for the CNN')--1e-5
cmd:option('-cnn_weight_decay', 0, 'L2 weight decay just for the CNN')--0

-- Evaluation/Checkpointing
cmd:option('-train_split', 'train', 'if running on MSCOCO images, which split to use: val|test|train')
cmd:option('-eval_split', 'val', 'if running on MSCOCO images, which split to use: val|test|train')
cmd:option('-batch_size_val',1, 'if > 0 then overrule, otherwise load from checkpoint.')
cmd:option('-val_images_use', 497, 'how many images to use when periodically evaluating the validation loss?')
cmd:option('-save_checkpoint_every', 250, 'how often to save a model checkpoint?')
...
cmd:option('-id', '_MSR_VTT_challenge', 'an id identifying this run/job. used in cross-val and appended when writing progress files')
...
-- Load MSR-VTT features
-------------------------------------------------------------------------------
local loader = DataLoader_FEAT{h5_file = opt.input_h5, json_file = opt.input_json, input_feature_dim = opt.input_feature_dim}
...
if string.len(opt.start_from) > 0 then
  ...
  protos.cnn = net_utils.build_cnn({encoding_size = opt.input_encoding_size, backend = cnn_backend, input_feature_dim = opt.input_feature_dim})
  ...
  protos.expander_ori = nn.FeatExpander(1)
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local data = loader:getBatch{batch_size = 1, split = split, seq_per_img = opt.seq_per_img, input_feature_dim = opt.input_feature_dim} --opt.batch_size
    -- data.images = net_utils.prepro(data.images, false, opt.gpuid >= 0) -- preprocess in place, and don't augment
    n = n + data.images:size(1)

    -- forward the model to get loss
    local FE = data.images
    local feats = protos.cnn:forward(FE:cuda())
    -- local feats = data.images
    ...
    local seq = protos.lm:sample(feats:cuda(), sample_opts)
    -- local seq = protos.lm:sample(feats:cuda())
...
local function lossFun()
  ...
  -----------------------------------------------------------------------------
  -- get batch of data  
  local data = loader:getBatch{batch_size = opt.batch_size, split = opt.train_split, seq_per_img = opt.seq_per_img, input_feature_dim = opt.input_feature_dim}
  ...
  --local images = torch.ByteTensor(opt.batch_size, 7168)
  local images = (data.images):cuda()
  ...
  if opt.finetune_cnn_after >= 0 and iter >= opt.finetune_cnn_after then
    ...
    local dx = protos.cnn:backward(images, dfeats) --data.images
  end
...
local best_score
local forward_iter = opt.update_iter
local ac_grad_params = 0
local ac_cnn_grad_params = 0
while true do  
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    local val_loss, val_predictions, lang_stats = eval_split(opt.eval_split, {val_images_use = opt.val_images_use})
    ...
    if lang_stats then
      ...
      current_score = lang_stats['CIDEr']  + lang_stats['METEOR'] + lang_stats['Bleu_4']+ lang_stats['ROUGE_L']
    else
  ...
  ac_grad_params = ac_grad_params + grad_params
  ac_cnn_grad_params = ac_cnn_grad_params + cnn_grad_params
  if iter%forward_iter == 0 then
    print('update parameter: iter ' .. iter)
