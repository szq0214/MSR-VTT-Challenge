...
local DataLoader_FEAT = torch.class('DataLoader_FEAT')

function DataLoader_FEAT:__init(opt)
  ...
  local images_size = self.h5_file:read('/FEAT'):dataspaceSize()
  assert(#images_size == 2, '/FEAT should be a 2D tensor')
  -- assert(images_size[3] == images_size[4], 'width and height must match')
  self.num_images = images_size[1]
  self.num_channels = images_size[2]
  -- self.max_image_size = images_size[3]
  print(string.format('read %d features of size %d', self.num_images, 
            self.num_channels))
...
function DataLoader_FEAT:resetIterator(split)
  self.iterators[split] = 1
end

function DataLoader_FEAT:getVocabSize()
  return self.vocab_size
end

function DataLoader_FEAT:getVocab()
  return self.ix_to_word
end

function DataLoader_FEAT:getSeqLength()
...
--]]
function DataLoader_FEAT:getBatch(opt)
  ...
  local img_batch_raw = torch.FloatTensor(batch_size, opt.input_feature_dim) 
  ...
  for i=1,batch_size do
    ...
    local img = self.h5_file:read('/FEAT'):partial({ix,ix},{1,self.num_channels})
    img_batch_raw[i] = img
