...
function net_utils.build_cnn(opt)
  ...
  cnn_part:add(nn.Linear(opt.input_feature_dim,4096, true))
  --cnn_part:add(nn.BatchNormalization(4096))
  cnn_part:add(backend.ReLU(true))
  --cnn_part:add(nn.Dropout(p1))
  cnn_part:add(nn.Linear(4096, encoding_size, true))
  --cnn_part:add(nn.BatchNormalization(encoding_size))
  cnn_part:add(backend.ReLU(true))
...
function net_utils.language_eval_test(predictions, id)
  -- this is gross, but we have to call coco python code.
  -- Not my favorite kind of thing, but here we go
  local out_struct = {val_predictions = predictions}
  utils.write_json('coco-caption/test' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('./misc/call_python_caption_eval_test.sh test' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('coco-caption/test' .. id .. '.json_out.json') -- god forgive me
  return result_struct
end
return net_utils
